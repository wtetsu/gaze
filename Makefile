...
check-cross-compile:
	...
	@file ${OUT}/gaze_macos_amd_${VERSION}/${BINARY_NAME}   | grep "Mach-O 64-bit"  | grep -c x86_64 | grep -q "1" || (echo "Error: macOS amd binary is not correctly built" && exit 1)
	@file ${OUT}/gaze_macos_arm_${VERSION}/${BINARY_NAME}   | grep "Mach-O 64-bit"  | grep -c arm64  | grep -q "1" || (echo "Error: macOS arm binary is not correctly built" && exit 1)
	@file ${OUT}/gaze_windows_${VERSION}/${BINARY_NAME}.exe | grep "MS Windows"     | grep -c x86-64 | grep -q "1" || (echo "Error: Windows binary is not correctly built"   && exit 1)
	@file ${OUT}/gaze_linux_${VERSION}/${BINARY_NAME}       | grep "ELF 64-bit LSB" | grep -c x86-64 | grep -q "1" || (echo "Error: Linux binary is not correctly built"     && exit 1)
